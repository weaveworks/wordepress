...
func createOrUpdatePosts(user, password, path, query, product, version, tag, dir string) {
	// Load local site
	localPosts, images, err := wordepress.ParseDir(product, version, tag, dir)
	if err != nil {
	...
	query = "context=edit&per_page=100&" + query
...
func deletePosts(user, password, path, query, product, version, tag string) {
	endpoint := baseURL + path
